if node['firewalld-rules']['rich']
  node['firewalld-rules']['rich'].each do |item|
    ...
    firewalld_rich_rule item['name'] do
      ...
      action if item['action'] == 'add' ? :add : :remove # Default to remove
    end
  end
end
...
if node['firewalld-rules']['service']
  node['firewalld-rules']['service'].each do |item|
    raise "firewalld-rules::service - Missing name" if !item.key?('name')
    raise "firewalld-rules::service - Missing zone" unless %w(add remove).include? item['action']
    firewalld_service item['name'] do
